...
const articlesPageSlice = createSlice({
  ...
  reducers: {
    ...
    initState: (state) => {
      const viewFromStorage = localStorage.getItem(
        ARTICLES_VIEW_LOCALSTORAGE_KEY,
      );
    ...
    setListScrollState: (state, action: PayloadAction<StateSnapshot>) => {
      state.listScrollState = action.payload;
    },
    setGridScrollState: (state, action: PayloadAction<GridStateSnapshot>) => {
      state.gridScrollState = action.payload;
    },
  },
    ...
    builder
      .addCase(fetchArticlesList.pending, (state, action) => {
        ...
        if (action.meta.arg.replace) {
          ...
          state.listScrollState = undefined;
          state.gridScrollState = undefined;
          state.virtuosoSession = (state.virtuosoSession ?? 0) + 1;
        }
      })
...
export const getListScrollState = (state: RootState) =>
  state.articlesPage?.listScrollState;

export const getGridScrollState = (state: RootState) =>
  state.articlesPage?.gridScrollState;

export const getVirtuosoSession = (state: RootState) =>
  state.articlesPage?.virtuosoSession ?? 0;
